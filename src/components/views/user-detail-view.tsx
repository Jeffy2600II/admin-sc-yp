...
export function UserDetailView({ userId }: UserDetailViewProps) {
    ...
    const controller = open({
      ...
      body: (
        <FormSection>
          <UserDeptSummary
            avatar={
              <Avatar name={user.full_name} size={48} avatarUrl={user.avatar_url ?? null} />
            }
            name={user.full_name}
            meta={
              <>
  ...
  // v1.7: user.color doesn't exist — use dept.color (or brand fallback) for hero gradient.
  const userColor = dept?.color || "#0EA5E9";
    ...
    <div className="page container">
      <AdminHero style={heroStyle} name={user.full_name}>
        <div className="admin-hero__content" style={{ textAlign: "center" }}>
          ...
          >
            <Avatar name={user.full_name} size={80} avatarUrl={user.avatar_url ?? null} />
          </div>
    ...
    <>
      <div className="admin-action-sheet__summary">
        <div className="admin-action-sheet__avatar">
          <Avatar name={user.full_name} size={56} avatarUrl={user.avatar_url ?? null} />
        </div>
